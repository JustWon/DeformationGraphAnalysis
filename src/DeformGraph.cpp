...
void DGraph::deform(TriMesh& d_mesh) {
    ...
    std::vector<Eigen::Vector3d> vs(this->node_pos.size());		//DongwonShin: graph node position
	...
	for (size_t i=0; i<verts.size(); ++i) {
		...
		// DongwonShin: 
		// R. W. Sumner, J. Schmid, and M. Pauly, ¡°Embedded deformation for shape manipulation,¡± ACM Trans. Graph., vol. 26, no. 3, p. 80, 2007.
		// it implies Equation (4).
		...
		// DongwonShin: weight normalization?
        for (int j=0; j<n_neigh; ++j)
